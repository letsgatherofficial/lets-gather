...
import { ChevronLeft, ChevronRight, CalendarDays, List, Lock, X } from "lucide-react";
              ...
              <div className="space-y-3">
                {selectedDaySlots.length ? (
                  selectedDaySlots.map((slot) => (
                    <SlotCard key={slot.id} slot={slot} showBookingLinks={showBookingLinks} onBook={handleBookSlot} />
                  ))
                ) : (
          ...
          <div className="space-y-3">
            {sortedSlots.length ? (
              sortedSlots.map((slot) => (
                <SlotCard key={slot.id} slot={slot} showBookingLinks={showBookingLinks} layout="list" onBook={handleBookSlot} />
              ))
            ) : (
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-100 p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">{selectedSlot.title}</p>
              </div>
              <button
                onClick={handleCloseModal}
                className="h-8 w-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition"
              >
                <X size={16} />
              </button>
            </div>
            <div className="p-6">
              <IntakeForm slotId={selectedSlot.id} slotDetails={selectedSlot} />
            </div>
          </div>
        </div>
      )}
...
function SlotCard({
  ...
  if (layout === "list") {
    return (
      <article
        className={cn(
          "glass rounded-2xl p-5 border border-slate-100/60 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 cursor-pointer transition hover:shadow-md",
          !isPublic && !isFull && "hover:border-brass/30"
        )}
        onClick={() => !isPublic && !isFull && onBook(slot)}
      >
        ...
        {showBookingLinks && slot.visibility === "appointment" && (
          isFull ? (
            ...
            <span className="inline-flex h-9 items-center gap-1.5 rounded-xl px-4 text-xs font-bold bg-brass/10 text-brass">
              Click to Book
            </span>
          )
        )}
  ...
  return (
    <article
      className={cn(
        "glass rounded-xl p-4 border border-slate-100/60 shadow-sm space-y-2 cursor-pointer transition hover:shadow-md",
        !isPublic && !isFull && "hover:border-brass/30"
      )}
      onClick={() => !isPublic && !isFull && onBook(slot)}
    >
      ...
      {showBookingLinks && slot.visibility === "appointment" && (
        isFull ? (
          ...
          <span className="inline-flex h-8 items-center gap-1 rounded-lg px-3 text-[10px] font-bold bg-brass/10 text-brass">
            Click to Book
          </span>
        )
      )}
